...
def train_model_jiang(model, dataloaders, dataset_sizes,ratio, type,pattern,criterion, optimizer, name,scheduler=None, num_epochs=100,rerun=False):
    if rerun == True:
        ...
        print(num_epochs)
        since = time.time()
        model.load_state_dict(torch.load('./pth/AlexNet/ratio=0/Weight/test_13.pth'))
        ...
        for epoch in range(13, num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                savePth = './pth/AlexNet/ratio=0/Weight'
                # deep copy the model
                if phase == 'val':
                    path =  savePth +'/test_{}.pth'.format(epoch + 1)
                    torch.save(model.state_dict(), path)
                    WriteData(savePth,
                              str((round(float(epoch_acc), 4)) * 100) + '%-' + 'epoch=' + str(epoch + 1) + '\n')

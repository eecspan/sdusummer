...
def train_model_jiang(model, dataloaders, dataset_sizes,ratio, type,criterion, optimizer, name,scheduler=None, num_epochs=100,rerun=False):
    if rerun == True:
        print('我进来了')
        print(num_epochs)
        since = time.time()
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                if phase == 'val' and epoch_acc > best_acc:
                    ...
                    torch.save(model.state_dict(), path)
                    WriteAccuracy(savePth, str((round(float(epoch_acc),4))*100) + '%-')

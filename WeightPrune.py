...
def weightPruneModelOp(model_name,batch_size,img_size,ratio,pattern,epoch,weightParameter,LinearParameter,operation = None):
    ...
    if pattern == 'retrain' or pattern == 'train' or pattern == 'DPTrain':
        if pattern == 'retrain':
            if operation == 'activationRetrainAfterWeightRetrain':
                getPth = './pth/' + model_name + '/ratio=0'  + '/Weight' + '/best.pth'
            else:
